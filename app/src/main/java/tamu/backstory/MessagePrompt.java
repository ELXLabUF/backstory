...
public class MessagePrompt extends FragmentActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ChapterDataSingleton.getSync(NewMainMenu.mContext).writeToFile(Environment.getExternalStorageDirectory().getAbsolutePath(), "levelselect.txt", levelTimestamp);
    ...
    public void setBackground(){
        ImageView background = (ImageView) this.findViewById(R.id.messagePromptBackground);
        System.out.println("missionNumber message prompt: " + missionNumber);
        //#offBy1Error, fixed
        //completely temporary, don't do this at home, kids
        if(missionNumber == 0) background.setBackground(getResources().getDrawable(R.drawable.gameified_backstory_story_background_1));
        if(missionNumber == 1) background.setBackground(getResources().getDrawable(R.drawable.gameified_backstory_story_background_2));
        if(missionNumber == 2) background.setBackground(getResources().getDrawable(R.drawable.gameified_backstory_story_background_3));
    }
