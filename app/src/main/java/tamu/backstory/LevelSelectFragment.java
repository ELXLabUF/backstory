...
public class LevelSelectFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if(Integer.parseInt(mParam1) == ChapterDataSingleton.getSync(NewMainMenu.mContext).getListOfMissionData().size() - 1){
            rightWhiteBlock.setVisibility(View.INVISIBLE);
        }
        TextView numberOfRecordings = (TextView) v.findViewById(R.id.numberOfRecordings);
        int currentRecordingPlace = ChapterDataSingleton.getSync(NewMainMenu.mContext).getListOfMissionData().get(Integer.parseInt(mParam1)).getFileNameDigit() - 1;
        System.out.println("currentRecordingPlace: " + currentRecordingPlace);
        numberOfRecordings.setText("" + ChapterDataSingleton.getSync(NewMainMenu.mContext).getNumberOfRecordings(currentRecordingPlace));

        int numberRecordings = ChapterDataSingleton.getSync(NewMainMenu.mContext).getNumberOfRecordings(currentRecordingPlace);
        //setting stars
        ...
        for(int i = 0; i < ChapterDataSingleton.getSync(NewMainMenu.mContext).getListOfNumberRecordings().size(); i++){
            System.out.println(i + "-- " + ChapterDataSingleton.getSync(NewMainMenu.mContext).getNumberOfRecordings(i));
        }
        ...
        if(prevLevelChecker == -1 && ChapterDataSingleton.getSync(NewMainMenu.mContext).getNumberOfRecordings(Integer.parseInt(mParam1)) < 3){ //base case, have to allow entry for the first level at all times
